...
export function NappyChangeForm({ onChange }) {
  ...
  const handleAddEntry = () => {
    ...
    const updatedEntries = [...nappyEntries, newEntry];
    setNappyEntries(updatedEntries);
    setCurrentStatus(""); // Reset after adding
    setDiaperApplied(false); // Reset diaper cream checkbox

    // Notify parent component about the new nappy entries
    if (onChange) {
      onChange(updatedEntries); // Pass updated entries to the parent component
    }
  };
  ...
  const handleDeleteEntry = (index) => {
    ...
    // Notify parent component about the updated nappy entries
    if (onChange) {
      onChange(updatedEntries); // Pass updated entries to the parent component
    }
  };
    ...
    <div className="mb-4">
      <div className="flex gap-2 mb-4">
        {["Wet", "Soiled", "Both"].map((status) => (
          <button
            ...
            className={`px-4 py-2 border rounded ${currentStatus === status ? "bg-zinc-900 text-white" : "bg-gray-100"}`}
          >
            {status}
          </button>
        ))}
      ...
      <div className="flex items-center gap-2 mb-4">
        <input
          ...
          id="diaperCream"
        />
        <label htmlFor="diaperCream" className="cursor-pointer">Apply Diaper Cream</label>
      </div>
        ...
        {nappyEntries.length > 0 && (
          <div className="space-y-2">
            {nappyEntries.map((entry, index) => (
              <div key={index} className="flex items-center justify-between p-2 border rounded bg-gray-100">
                <div className="flex gap-2">
                  <span className="font-bold">{entry.time}</span>
                  <span
                    className={`px-2 py-1 rounded ${entry.status === "Wet" ? "bg-blue-100 text-blue-800" :
                        entry.status === "Soiled" ? "bg-yellow-100 text-yellow-800" : "bg-purple-100 text-purple-800"
                      }`}
                  >
                    {entry.status}
                  </span>
                  {entry.cream && (
                    <span className="px-2 py-1 bg-green-100 text-green-800 rounded">Cream</span>
                  )}
                </div>
